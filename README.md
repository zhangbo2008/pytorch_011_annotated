...
setup.py里面cuda部分也都删了,方便观看.

    把握整体项目代码结构!
    这个脚本里面第一步会运行:build_deps生成TH,THcc
    然后他俩被封装成库,库里面的就不能被调试了.
    库被csrc里面带有pyobject的c代码调用. 调用的衔接是TensorMethods.cwrap
    csrc里面的这种c代码可以被python调用,也可以调试.

先看TH库
理解代码里面tensor
    注意tensor的所有操作,入参出餐都是tensor!!!!!!!!!!
    torch/lib/TH/generic/THTensor.c


    理解tensor结构体:
    https://blog.csdn.net/EMIvv/article/details/122590890  
    https://www.jianshu.com/p/ebd7f6395bf4

    特别是里么的storageoffset和stride 2个参数很重要!
...
contiguous理解:
    https://blog.csdn.net/qq_40206371/article/details/120617740
    具体实现:THTensor.c:189, 更底层是用181行, 再底层就是rawResize之后重新copy数据,所以就连续化了.

在看csrc里面文件.

    python setup.py clean 可以清楚非源码,更专注核心.



torch/autograd看计算图如何实现.
    大概看懂流程之后.


下一步就是debug test文件夹里面的代码.
    test/test_autograd.py

        bp核心算法:
            /usr/local/lib/python2.7/dist-packages/torch/autograd/engine.py:40
