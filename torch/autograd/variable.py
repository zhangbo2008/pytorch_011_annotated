...
#torch的变量信息. 计算图,每一个变量都是leaf. 通过运算链接起来.
class Variable(object):
    ...
    def __init__(self, tensor, creator=None, requires_grad=True):
        if creator is None:
            creator = Leaf(self, requires_grad) # 表示他的父节点.这行说如果没有指定父节点就自己作为父节点.
        self.data = tensor
        self.creator = creator
        self._grad = None

    @property
    def grad(self): # 表示这个是否计算梯度.
        if self.creator.requires_grad:
            # TODO: this won't have to be zeroed in the future
            self._grad = self._grad or self.data.new(self.data.size()).zero_()  # 如果需要计算grad,那么就需要生成grad这个tensor.初始化他都是0.
        return self._grad
    ...
    def __getitem__(self, key):
        return Index(key)(self)[0]  # self[key][0]

    def backward(self, gradient=None):
        if gradient is None:
            if self.data.numel() != 1:    # numel:number element
                raise RuntimeError('backward should be called only on a scalar (i.e. 1-element tensor) or with gradient w.r.t. the variable')
            gradient = self.data.new(1).fill_(1) # 梯度就是1.
        self._execution_engine.run_backward(self, gradient) # 然后继续调用backward
    ...
    def type(self, t):
        ...
        return self
#变量的操作.每一个操作也用forwar,backward实现.
    def add(self, other):
