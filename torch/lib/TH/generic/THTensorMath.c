...
void THTensor_(add)(THTensor *r_, THTensor *t, real value)
{
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) && THTensor_(nElement)(r_) == THTensor_(nElement)(t)) {//如果r_,t 都是行向量,并且元素个数一样.那么直接加即可.
      real *tp = THTensor_(data)(t);
      real *rp = THTensor_(data)(r_);
      long sz = THTensor_(nElement)(t);
      long i;//通过这个测试可以得知#pragma omp parallel for这个代码意思是：接下来的这个for循环将被多个线程同时运行的，也就是多个线程同时运行一个for循环,用来给for循环加速.
...
//// t里面的数据那个是最大的. 注意下面使用了宏t_data表示t的data属性的值. 找到的索引写入indices, 数值写入values里面. 注意tensor的所有操作,入参出餐都是tensor!!!!!!!!!!
void THTensor_(max)(THTensor *values_, THLongTensor *indices_, THTensor *t, int dimension)
{
  ...
  THLongStorage_set(dim, dimension, 1);//dim.data[dimensiion]=1
  THTensor_(resize)(values_, dim, NULL);//values和indices都设置为1位. THStorage.c:194
  THLongTensor_resize(indices_, dim, NULL);
  THLongStorage_free(dim);
// t里面的数据那个是最大的. 注意下面使用了宏t_data表示t的data属性的值. 找到的索引写入indices, 数值写入values里面.
...
void THTensor_(min)(THTensor *values_, THLongTensor *indices_, THTensor *t, int dimension)
{
  ...
  dim = THTensor_(newSizeOf)(t);
  THLongStorage_set(dim, dimension, 1);//把dim这个tensor的dimension设置为1.
