...
*/// t_: input data ||  ir, ic: data rows, cols  ||  kr, kc: kernel rows, cols  ||  sr, sc: stride rows, cols   这个是kernal运算时候的每次偏移量,不是tensor里么的stride,而是conv运算的stride|| r_: output data ||k_:kernal data
void THTensor_(validXCorr2Dptr)(real *r_,
                                       ...
                                       long sr, long sc)
{
  long or = (ir - kr) / sr + 1;        //行计算次数
  long oc = (ic - kc) / sc + 1;         //列计算次数
  ...
  if ((sc != 1) || (oc < 4))  {
    ...
    for(yy = 0; yy < or; yy++) {
      for(xx = 0; xx < oc; xx++) {
        /* Dot product in two dimensions... (between input image and the mask) */  //这个for循环算两个矩阵的dot.
        real *pi_ = t_ + yy*sr*ic + xx*sc;  //图像矩阵的左上角.
        real *pw_ = k_; //kernal做上jiao
        real sum = 0;
        for(ky = 0; ky < kr; ky++) {
          for(kx = 0; kx < kc; kx++) {
            sum += pi_[kx]*pw_[kx];
    ...
    /* SSE-based convolution */ //图像大的话走这个算法.道理一样估计底层就是simd加速.
    for(yy = 0; yy < or; yy++) {
      ...
      for (ky = 0; ky < kr; ky++) {
        real *pis_ = pi_;
        for (kx = 0; kx < kc; kx++) {
          THVector_(add)(r_, pis_, alpha*pw_[kx], oc); //THvector.c:20
          pis_++;
        }
        pi_ += ic; /* next input line */
        pw_ += kc; /* next mask line */
...
/* //“REV缩写在软件中REV是指修订本的意思，revision的简写，
...
/* 图像t_, kernal k_ 返回值r_ ,这个是conv2d的再封装,让他可以算三维向量.
  3D input, 3D kernel, 4D output
  like rank1 update
  A <- xx' + beta*A
*/
...
#pragma omp parallel for private(k)
  for(k = 0; k < nKernelPlane; k++)
  {
    ...
    for(i = 0; i < nInputPlane; i++)
    {
      /* get output */ //ptr_output指向一个output_data的地址,然后下面的代码填充这个地址里面的内容, 所以我们得到output_data
      real *ptr_output = output_data + k*nInputPlane*nOutputCols*nOutputRows + i*nOutputCols*nOutputRows;
      /* get input */
      real *ptr_input = input_data+i*istride0;
