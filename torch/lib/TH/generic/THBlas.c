...
#define TH_GENERIC_FILE "generic/THBlas.c"  //BLAS（Basic Linear Algebra Subprograms）
#else
...
#if defined(TH_REAL_IS_DOUBLE)   //如果使用了blas库包,那么么使用库包里面的这2个函数即可.
...
//x 向量,每间隔incx乘以a再放回原来位置. n是做n次这个运算.
...
// 这个是 ge表示general m matrix  v vector表示 矩阵和向量的相乘.

//    THBlas_(gemv)('n', mat->size[0], mat->size[1],
                  // alpha, THTensor_(data)(mat), mat->stride[1],
                  // THTensor_(data)(vec), vec->stride[0],
                  // beta, THTensor_(data)(r_), r_->stride[0]);
// n表示不取转职,m行数,n列数,a矩阵,lda是矩阵步长,x向量,incx: x的步长,y向量, incy:y步长.y=beta*y+alpha*a*x
    ...
    if( (trans == 'T') || (trans == 't') )//矩阵乘以向量.
    {
      for(i = 0; i < n; i++)
      {
        ...
        if (beta == 0)
          ...
          y[i*incy] = beta*y[i*incy] + alpha*sum; //y=beta*y+alpha*a*x
      }
    }
    else
    {//这部分是向量乘以矩阵.
      if(beta != 1) //
        THBlas_(scal)(m, beta, y, incy);//调用刚才的scal方法.
      
      for(j = 0; j < n; j++)//n是列数量.
      {
        real *column_ = a+lda*j; //column是第j列
