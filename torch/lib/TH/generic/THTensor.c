...
//src[dimension][firstIndex:firstindex+size],进行压缩. https://blog.csdn.net/EMIvv/article/details/122590890  https://www.jianshu.com/p/ebd7f6395bf4
void THTensor_(narrow)(THTensor *self, THTensor *src, int dimension, long firstIndex, long size)
{
  ...
  if(firstIndex > 0)// 初始index,改变offset即可.
    self->storageOffset += firstIndex*self->stride[dimension];

  self->size[dimension] = size;//改变size即可.
}
//选择维度压缩成一个index
...
static void THTensor_(rawResize)(THTensor *self, int nDimension, long *size, long *stride)
{
  ...
  int nDimension_;
  long totalSize;
  int hascorrectsize = 1;// 默认是不需要被修改.这个变量等于1表示size已经正确了.
  nDimension_ = 0;
  for(d = 0; d < nDimension; d++) //这个for是检测里面的size和stride,如果有一个跟变化钱不同那么,就需要修改设置  hascorrectsize = 0;  
  {
    if(size[d] > 0)
    {
      nDimension_++;
  ...
  nDimension = nDimension_;//这变量是修改后的维度.
  ...
  if(nDimension > 0)//如果修改后维度大于0,那么进行修改.
  {
    if(nDimension != self->nDimension)// malloc开好数组大小.
    {
    ...
    for(d = self->nDimension-1; d >= 0; d--)
    {
      self->size[d] = size[d];//更新size大小.
      if(stride && (stride[d] >= 0) )
        self->stride[d] = stride[d];//更新stride
      else
      {//如果新的stride是负数.那么就是他的下一个的倍数.
        if(d == self->nDimension-1)
          self->stride[d] = 1;
        else
          self->stride[d] = self->size[d+1]*self->stride[d+1];
      }
      totalSize += (self->size[d]-1)*self->stride[d];// 统计所有的cpp占用大小.
    }
...
//desc是description的意思.  效果就是打印tensor的size大小.
...
#define _stringify(x) #x
  n += snprintf(str, L-n, "torch." _stringify(x) "Tensor of size "); // snprintf 按照format的格式格式化为字符串，然后再将其拷贝至str中。
#undef _stringify
  int i;
