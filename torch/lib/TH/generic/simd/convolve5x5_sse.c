...
/* SSE variants */  //SSE指令集系列——Streaming SIMD Extensions，单指令多数据流扩展  // weight卷积核. image是图像展开成一维向量.
void convolve_5x5_1_sse(float* output, float* image, float* weight, long count, long outputStride, long inputStride) {
  long i = 0;
  long alignedCount4 = count & 0xFFFFFFFC;//0xFFFFFFFC表示-4 二进制1111111111 1100. 说明就是count去掉最后两位.也就是裁剪到4的倍数.
  DECLARE_OUTPUT_1() //展开后: float* output0 = output;
  for (; i < alignedCount4; i+=4) {// 这个算完4的倍数部分的卷积.
    CONVOLVE_4COLS_XROWS(1, i)
  }
  for (; i < (count); i++) {//计算超出4倍数的剩余部分的卷积.
