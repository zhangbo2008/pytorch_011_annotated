...
// simd是cpp的优化.具体要看intel指令集.
/* Weights */  //_mm256_set1_ps()  其中 simd_type 表示类型, 也就是m256, inst_var表示操作set1_ps. q表示填充的数的指针. 所以结果是创造一个8float向量. 里面数据都用*q填充.
#define LOAD_WEIGHT(q, simd_type, inst_var) _m ## simd_type ## inst_var(*(q))
//设置5个向量.类型都是simd_type, 比如m256.
...
#define DECLARE_SUMX(rows) DECLARE_SUM_ ## rows  //这个是作为拼接函数用的. 给row,然后返回DECLARE_SUM_row这个函数名字.
...
// 下面作为例子我们解析CONVOLVE_4COLS_XROWS 的组成. 
