...
{
    long *size; //size是一个数组. eg:[2,3,4] 三维向量.
    long *stride;   //stride[i]表示第i个维度上索引的底层间隔! eg: stride[2]表示THtensor第二个维度上索引i-索引i-1的底层cpp间隔.
    int nDimension;  
    
    THStorage *storage;
    long storageOffset;//返回tensor的第一个元素与其storage的第一个元素的偏移量。
    int refcount;
