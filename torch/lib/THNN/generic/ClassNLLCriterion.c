          ...
          THTensor *total_weight)
{
  ...
  input = THTensor_(newContiguous)(input); //为什么要连续化?
  ...
  if (THTensor_(nDimension)(input) == 1) {
    int cur_target = target_data[0] - TH_INDEX_BASE;//以为targetdata,索引是从1开始算的.底层我们把他恢复成0开始.
    THAssert(cur_target >= 0 && cur_target < n_classes);
    total_weight_data[0] = weights ? weights_data[cur_target] : 1.0f; //如果有权重就用,没有就用默认值1.
    output_data[0] = -input_data[cur_target] * total_weight_data[0]; //公式 output= -yipi, yi是cur_target
  } else if (THTensor_(nDimension)(input) == 2) {// 说明有batchsize
    int batch_size = THTensor_(size)(input, 0);   //第零维度是batchsize.
    ...
    for (i = 0; i < batch_size; i++) {
      ...
      output_data[0] -= input_data[i * n_target + cur_target] * cur_weight;// 因为input_data是一维数组,[i * n_target + cur_target] 算偏移量即可.
    }
  }
          ...
          THTensor *total_weight)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    gradInput_data[cur_target] =
      (!sizeAverage && weights) ? -weights_data[cur_target] : -1;//梯度也容易,就是负的权重,或者-1
